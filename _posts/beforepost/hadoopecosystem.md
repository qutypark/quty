...
>>distribute the storage of big data across cluster of computers> <br>
-> it makes all hard drives on cluster look like one giant file system><br>
it actually maintains redundant copies of that data><br>
it can actually recover from that and it will back itself up to  a backup copy><br>
...
>> (on top of HDFS)
managing the resoures on computing cluster = resources negotiator 
it ddecieds what gest s to run task when waht nodes are available for extra work which nodes are or not availabe 
like heartbeat make cluster keep going
...
>> (on top of YARN)
Mappers: ability to transform your data in parallel across your entire computing cluster in a very efficient manner
Reducers: aggregate taht data together <br>
...
pigs actullay trasnform that script into something that will run on map reduce
which goes through yarn and HDFS to process and get the data needed <br>
...
>> actually execute sql queries on the data stored on hadoop cluster even though it not a really relational database
...
>> gives you a view of actual state of cluster and can visualize that is running on your cluster which system you are using
>> how much resources 
>> can execute hive quiereis or pig queries or import database into hive etc
...
>> run qureies on your data like mapreduce
>> versatile cause it handle like SQL and also machine learning across entire cluster
>> can handel streaming data in real time
...
>> (on top of YARN and Mesos) = same level of MapReduce <br>
>> using same tech as spark notably with Directed acyclic graph
>> it can produce more optimal plans for executing queries 
>> used in conjunction with hive to accelarate it
>> hive through tez often be faster than hive through MapReduce
...
>> very very fast DB meant for very large transaction rates
>> HBase can expose data stored on cluster and transformed in some way by spark or MapReduce
>> fast way of exposing results to other system
...
>> processing streaming data from sensors or web logs in real time
>> so it doesn't have to be a batch 
>> you can update machinge learning model or transform data into a database all in realtime as it comes in 
...
>> (sit off to the side)<br>
>> if you have a task needs to happend on cluster involving different steps on different systems,
>> Oozie is a way of scheduling all of these things together into jobs that can be run on some sort of schedule
>> when you have more complicated operations requiring loading data into hive and integrating with pig
>> and querying it with spark and transforming result into HBase...
>> Oozie can manage it above 
...
>> (sit off to the side)<br>
>> can be used for keeping track of which node are up or down
>> reliabe way of keeping track of shared states across cluster that different application can use
>> application mentioned above rely on Zookeeper to maintain reliable and consistent performance across cluster
...
>> (sit off to the side)<br>
>> how do we actually get data into cluster and onto HDFS from external datasource
...
###### 2.11.2 Flume <br>
>> transporting web logs at a very large scale and very reliable to your clutser
>> can listen to web logs from web servers and publish them into your cluster in real time
>> for processing by storm or spark streaming
...
###### 2.11.2 Kafka <br>
>> solve similar problem to flume but bit in more general purpose
